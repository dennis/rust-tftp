            ...
            Ok((amt, src)) => {
                ...
                match TftpEnDecoder::decode(&buf[..amt]) {
                    Ok(Packet::RRQ(filename, mode_name)) => {
                        ...
                        if filename == "hello" {
                            out = TftpEnDecoder::encode(Packet::Data(1, u8_array_to_vec("world".to_string().as_bytes()))).unwrap();
                        }
                        else {
                            out = TftpEnDecoder::encode(Packet::ERROR(ErrorCode::FileNotFound, "Test".to_string())).unwrap();
                        }
